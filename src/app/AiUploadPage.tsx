...
async function renderPdfToBase64(file: File, onProgress: (page: number, total: number) => void) {
...
export default function AiUploadPage() {
  ...
  const ingest = useCallback(async (file: File) => {
    ...
    try {
      ...
      for (const img of images) {
        const blob = await fetch(`data:image/png;base64,${img}`).then((r) => r.blob());
        form.append("images", blob, `page-${images.indexOf(img) + 1}.png`);
      }
  ...
  const dropLabel = useMemo(() => {
    if (busy) {
      if (progress) return `Reading page ${progress.page}/${progress.total}…`;
      return "Preparing PDF…";
    }
    if (isDragging) return "Release to send to gemma4:cloud";
    ...
    >
      ...
      >
        <Box>
          ...
          >
            ...
            gemma4:cloud via your Ollama API key. The structured result is
            shown for review before adding to your dashboard.
          </Typography>
        </Box>
      </Stack>
      ...
      {stats ? (
        <Box className="fade-up">
          <ConfirmedCard stats={stats} onReplace={onReplace} commitAction={commit} />
        </Box>
      ) : (
        <Box
          role="button"
        ...
        >
          ...
          <Box sx={{ position: "relative", zIndex: 1 }}>
            ...
            >
              {busy
                ? "Reading PDF pages and querying gemma4:cloud"
          ...
          >
            PDF is processed in the browser. API key stays on the server.
          </Typography>
        </Box>
      )}
